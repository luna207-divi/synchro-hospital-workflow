...
export const PatientDetailPanel = ({ patient: rawPatient, onClose, onUpdateStatus }) => {
  ...
  const getEnrichedPatient = (input) => {
    ...
    return {
      ...
      phone: input.phone || '',
      condition: input.condition || input.procedure || '',
      procedure: input.procedure || input.condition || '',
      priority: input.priority || 'Routine',
      workflowStage: input.workflowStage || 'ADMITTED',
      admissionStatus: input.admission_status || input.status || 'ADMITTED',
      admissionDate: input.admissionDate || '',
      department: input.department || '',
      preOpBay: input.preOpBay || input.assigned_bed ? `${input.assigned_bed?.room?.room_number || ''} / ${input.assigned_bed?.bed_number || ''}` : '',
      consultant: input.assigned_doctor || input.consultant || '',
      surgeon: input.assigned_doctor || input.surgeon || '',
      anesthesiologist: input.anesthesiologist || '',
      nurse: input.nurse || '',
      readinessScore: input.readinessScore || 0,
      otSuite: input.otSuite || '',
      scheduledTime: input.scheduledTime || '',
      cssdKit: input.cssdKit || '',
      cssdPackId: input.cssdPackId || '',
      cssdSterilization: input.cssdSterilization || '',
      cssdExpiry: input.cssdExpiry || '',
      cssdLocation: input.cssdLocation || '',
      cssdStatus: input.cssdStatus || '',
      recoveryBay: input.recoveryBay || '',
